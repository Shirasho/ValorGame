			...
			else
			{
				if (UGameViewportClient* ViewportClient = GetWorld()->GetGameViewport())
				{
					if (ViewportClient->Viewport && ViewportClient->Viewport->IsForegroundWindow()/*->IsFocused(ViewportClient->Viewport)*/)
					{
						...
						VALOR_LOG("Mouse X - %f", ViewportSize.X);
					...
					// Horizontal
						...
						{
							FVector Result = FMath::Lerp(FVector::ZeroVector, FVector(-VALOR_CAMERA_PAN_SPEED, 0.f, 0.f), FMath::Clamp(((float)CursorMoveZoneSizeHorizontal - MousePosition.X) / (float)CursorMoveZoneSizeHorizontal, 0.f, 1.f)) * DeltaTime;
							SetActorLocation(GetActorLocation() + Result);
						}
						else if (MousePosition.X >= ViewportSize.X - CursorMoveZoneSizeHorizontal)
						{
							FVector Result = FMath::Lerp(FVector::ZeroVector, FVector(VALOR_CAMERA_PAN_SPEED, 0.f, 0.f), FMath::Clamp((MousePosition.X - (float)ViewportSize.X + CursorMoveZoneSizeHorizontal) / (float)CursorMoveZoneSizeHorizontal, 0.f, 1.f)) * DeltaTime;
							SetActorLocation(GetActorLocation() + Result);
						}
						...
						{
							FVector Result = FMath::Lerp(FVector::ZeroVector, FVector(0.f, -VALOR_CAMERA_PAN_SPEED, 0.f), FMath::Clamp(((float)CursorMoveZoneSizeVertical - MousePosition.Y) / (float)CursorMoveZoneSizeVertical, 0.f, 1.f)) * DeltaTime;
							SetActorLocation(GetActorLocation() + Result);
						}
						else if (MousePosition.Y >= ViewportSize.Y - CursorMoveZoneSizeVertical)
						{
							FVector Result = FMath::Lerp(FVector::ZeroVector, FVector(0.f, VALOR_CAMERA_PAN_SPEED, 0.f), FMath::Clamp((MousePosition.Y - (float)ViewportSize.Y + CursorMoveZoneSizeVertical) / (float)CursorMoveZoneSizeVertical, 0.f, 1.f)) * DeltaTime;
							SetActorLocation(GetActorLocation() + Result);
						}
					}
				}
