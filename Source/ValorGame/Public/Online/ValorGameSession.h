...
#include "Online.h"
#include "ValorGameSession.generated.h"


struct FValorGameSessionParams
{
	/* The name of the session. */
	FName SessionName;
	/* Whether the match is a LAN Match */
	bool bIsLan;
	/* Whether this is a presence enabled session. */
	bool bIsPresence;
	/* The Id of player initiating the lobby. */
	TSharedPtr<const FUniqueNetId> UserId;
	/* The current best search result choice to join. */
	int32 BestSessionIdx;

	FValorGameSessionParams()
...
UCLASS(Config = Game)
class VALORGAME_API AValorGameSession : public AGameSession
{
...
protected:
	...
	FValorGameSessionParams CurrentSessionParams;
	/** Current host settings */
	TSharedPtr<class FValorOnlineSessionSettings> HostSettings;
	/** Current search settings */
	TSharedPtr<class FValorOnlineSearchSettings> SearchSettings;

	virtual void SetHostSettingsValues(TSharedPtr<class FValorOnlineSessionSettings> Settings, const FString& GameType, const FString& MapName);
	...
	*/
	DECLARE_EVENT_TwoParams(AValorGameSession, FOnCreatePresenceSessionComplete, FName /*SessionName*/, bool /*bWasSuccessful*/);
	...
	*/
	...
	DECLARE_EVENT_OneParam(AValorGameSession, FOnJoinSessionComplete, EOnJoinSessionCompleteResult::Type /*Result*/);
	FOnJoinSessionComplete JoinSessionCompleteEvent;

	/*
	* Event triggered after session search completes
	*/
	//DECLARE_EVENT(AValorGameSession, FOnFindSessionsComplete);
	DECLARE_EVENT_OneParam(AValorGameSession, FOnFindSessionsComplete, bool /*bWasSuccessful*/);
